...
- (void)configChartInfo
{
    ...
    if (!_chartView) {
        ...
        _percentageLabel.font = [UIFont systemFontOfSize:14.f];
...
- (void)setPercentageLabelText:(CJPieChartModel *)chartModel
{
    ...
    if (chartModel == nil) {
        ...
        if ([chartModel isKindOfClass:[CJPieChartModel class]]) {
            self.percentageLabel.text = [NSString stringWithFormat:@"%.01f%%", (chartModel.endPercentage - chartModel.startPercentage) * 100];
        }
    }
}
