//
//  ParkEnterprisesDetailViewController.m
//  eia
//
...
@interface ParkEnterprisesDetailViewController ()

@end

@implementation ParkEnterprisesDetailViewController
...
- (void)setTopView:(int)index {
    if (index == 0) {
        [self.eiaInformationTopView setFirstSelected];
    } else if (index == 1) {
        [self.eiaInformationTopView setSecondSelected];
    } else if (index == 2) {
        [self.eiaInformationTopView setThirdSelected];
    } else if (index == 3) {
        [self.eiaInformationTopView setFourSelected];
    } else if (index == 4) {
        [self.eiaInformationTopView setFiveSelected];
    } else if (index == 5) {
        [self.eiaInformationTopView setSixSelected];
    }
}
...
- (void)productionAction {
    UpdateProductionViewController *updateProductionVC = [[UpdateProductionViewController alloc] init];
    updateProductionVC.dicDetail = self.dicDetail;
    [updateProductionVC setReloadBlock:^(NSString * _Nonnull type) {
        [self setDetailNet:@"reloadProduction"];
    }];
    [self.navigationController pushViewController:updateProductionVC animated:YES];
}
...
- (void)setAllowDetailView:(NSDictionary *)dic {
    ...
    self.eiaInformationTopView = [[NSBundle mainBundle] loadNibNamed:@"EiaInformationTopView" owner:nil options:nil][0];
    self.eiaInformationTopView.frame = CGRectMake(0, 0, kDeviceWidth, 70);
    self.eiaInformationTopView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:self.eiaInformationTopView];
    [self.eiaInformationTopView.fBtn addTarget:self action:@selector(topBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.eiaInformationTopView.sBtn addTarget:self action:@selector(topBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.eiaInformationTopView.tBtn addTarget:self action:@selector(topBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.eiaInformationTopView.frBtn addTarget:self action:@selector(topBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.eiaInformationTopView.fiBtn addTarget:self action:@selector(topBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.eiaInformationTopView.sixBtn addTarget:self action:@selector(topBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    
    
    self.scrollViewAll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, self.eiaInformationTopView.frame.origin.y + self.eiaInformationTopView.frame.size.height, kDeviceWidth, kDeviceHeight - self.eiaInformationTopView.frame.origin.y - self.eiaInformationTopView.frame.size.height - NavRect - StatusRect)];
    self.scrollViewAll.backgroundColor = [UIColor whiteColor];
    self.scrollViewAll.contentSize = CGSizeMake(kDeviceWidth * 6, 0);
    ...
    self.productionProcessDetailView = [[ProductionProcessDetailView alloc] initWithFrame:CGRectMake(kDeviceWidth * 2 + 5, 0, kDeviceWidth - 10, self.scrollViewAll.frame.size.height - 115)];
    self.productionProcessDetailView.backgroundColor = [UIColor whiteColor];
    [self.productionProcessDetailView setAllowProductionProcessDetailView:dic];
    [self.scrollViewAll addSubview:self.productionProcessDetailView];
    
    ParkEnterprisesDetailBottomView *parkEnterprisesDetailBottomViewProduction = [[NSBundle mainBundle] loadNibNamed:@"ParkEnterprisesDetailBottomView" owner:nil options:nil][0];
    parkEnterprisesDetailBottomViewProduction.frame = CGRectMake(kDeviceWidth * 2 + 5, self.productionProcessDetailView.frame.origin.y + self.productionProcessDetailView.frame.size.height, kDeviceWidth - 10, 280);
    parkEnterprisesDetailBottomViewProduction.backgroundColor = [UIColor whiteColor];
    parkEnterprisesDetailBottomViewProduction.allAddPatrolBtn.hidden = YES;
    parkEnterprisesDetailBottomViewProduction.allAddPatrolBtn.hidden = YES;
    [parkEnterprisesDetailBottomViewProduction.updateBtn setTitle:@"更新 生产工艺 档案" forState:UIControlStateNormal];
    parkEnterprisesDetailBottomViewProduction.timeLabel.text = [NSString stringWithFormat:@"企业环保档案最近更新于  %@", [[NSString stringWithFormat:@"%@", dic[@"lastUpdateTime"]] timeToyyyyMMddHHmmssString]];
    [parkEnterprisesDetailBottomViewProduction.updateBtn addTarget:self action:@selector(productionAction) forControlEvents:UIControlEventTouchUpInside];
    [parkEnterprisesDetailBottomViewProduction.addPatrolBtn addTarget:self action:@selector(addAction) forControlEvents:UIControlEventTouchUpInside];
    [self.scrollViewAll addSubview:parkEnterprisesDetailBottomViewProduction];
    ...
    parkEnterprisesDetailBottomViewPollution.frame = CGRectMake(kDeviceWidth * 3 + 5, self.pollutionDetailView.frame.origin.y + self.pollutionDetailView.frame.size.height, kDeviceWidth - 10, 100);
    ...
    self.imageInformationView = [[ImageInformationView alloc] initWithFrame:CGRectMake(kDeviceWidth * 4, 0, kDeviceWidth, self.scrollViewAll.frame.size.height - 115)];
    ...
    parkEnterprisesDetailBottomViewImage.frame = CGRectMake(kDeviceWidth * 4 + 5, self.imageInformationView.frame.origin.y + self.imageInformationView.frame.size.height, kDeviceWidth - 10, 100);
    ...
    self.patrolRecordView = [[PatrolRecordView alloc] initWithFrame:CGRectMake(kDeviceWidth * 5 + 5, 0, kDeviceWidth - 10, self.scrollViewAll.frame.size.height - 115)];
    ...
    parkEnterprisesDetailBottomViewTable.frame = CGRectMake(kDeviceWidth * 5 + 5, self.patrolRecordView.frame.origin.y + self.patrolRecordView.frame.size.height, kDeviceWidth - 10, 100);
...
- (void)setReload:(NSString *)str dic:(NSDictionary *)dic{
    ...
    if ([str isEqualToString:@"reloadBasic"]) {
        ...
        [self.eiaInformationDetailView setAllowEiaInformationDetailView:dic];
    } else if ([str isEqualToString:@"reloadProduction"]) {
        [self.productionProcessDetailView removeFromSuperview];
        self.productionProcessDetailView = [[ProductionProcessDetailView alloc] initWithFrame:CGRectMake(kDeviceWidth * 2 + 5, 0, kDeviceWidth - 10, self.scrollViewAll.frame.size.height - 115)];
        self.productionProcessDetailView.backgroundColor = [UIColor whiteColor];
        [self.productionProcessDetailView setAllowProductionProcessDetailView:dic];
        [self.scrollViewAll addSubview:self.productionProcessDetailView];
